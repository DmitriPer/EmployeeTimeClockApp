...
<template>
  <BaseModal
    v-model:open="open"
    :title="existing ? 'Edit Pending Request' : 'Request Entry Edit'"
    :subtitle="`${toTimeInput(entry.clockInAt)} — ${entry.clockOutAt ? toTimeInput(entry.clockOutAt) : 'open'}`"
    @close="emit('close')"
  >
    <div class="space-y-4">
      <!-- Clock in / out -->
    ...
    <template #footer>
      <button
        ...
        @click="handleDelete"
        :disabled="deleting"
        class="mr-auto text-sm text-red-500 hover:text-red-700 disabled:opacity-50"
      >
